...
public class MorpionSwing extends ControleurMorpionClasse implements VueMorpion {
...
// ----------------------------------------------------
	...
	/** Quitter le jeu. */
	public void quitter() {
		this.getModele().quitter();
	}

	/** Cocher la case (x, y). */
	public void cocher(int x, int y) {
		try {
			this.getModele().cocher(x, y);
...
// Implantation des opérations de la vue
// -------------------------------------

	public void majCase(int x, int y, ModeleMorpion.Etat contenu) {
		cases[x][y].setIcon(images.get(contenu));
	}

	public void majJoueur(ModeleMorpion.Etat j) {
		joueur.setIcon(images.get(j));
	}
	...
	public void proclamerVainqueur(ModeleMorpion.Etat joueur) {
		JOptionPane.showMessageDialog(null, "gagne !", "Résultats",
				JOptionPane.INFORMATION_MESSAGE, images.get(joueur));
	}

	@Override
	public void terminer() {
		System.out.println("Arrêt de MorpionSwing " + fenetre.getTitle());
	...
	@Override
	protected void signalerErreur(String message) {
